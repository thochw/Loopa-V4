...
struct FriendsGroupsView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack {
                ...
                .opacity(variant == .travelers ? (1 - exploreSheetFraction(geometry: geometry)) : 1)
                .allowsHitTesting(variant == .travelers ? exploreSheetFraction(geometry: geometry) < 0.5 : true)
    ...
    private var inSheetDestinationSearchBar: some View {
        HStack(spacing: 12) {
            Image(systemName: "magnifyingglass")
                .font(.system(size: 16, weight: .medium))
                .foregroundStyle(.secondary)

            TextField("Search for your next destination", text: $searchText)
                .font(.app(size: 15, weight: .medium))
                .foregroundStyle(.primary)
                .textInputAutocapitalization(.never)
                .autocorrectionDisabled()
